...
{
//  int serport = serialOpen ("/dev/ttyS0", 9600);
  std::string line;
  std::string line2;
  std::string new1;
  std::string new2;
...
#endif
//	char buffer[100];
//	ssize_t length = read(serport, &buffer, sizeof(buffer));
//	buffer[length] = '\0';
//	new1=buffer;
        ifstream  myfile ("/home/pi/gps.txt");
...
/*       ofstream myfile3("/home/pi/data.txt");
       if (myfile3.open()){
        myfile3 << " data " << data1<< " " << data2 <<std::endl;   usleep(1000000);}
 */
//////////////////////////////////////////////
/*        ifstream myfile3 ("/home/pi/data.txt");
        // Send a message to rf95_server
        while (getline (myfile3,line3)){
         new3=line3;
        }
*/
        //const uint8_t *data3 = reinterpret_cast<const uint8_t*>(new3.c_str());
        uint8_t len1 = 100;
        uint8_t len2 = 100;
        uint8_t len3 = 100;
        printf("Send to => ", len1, len2, RF_GATEWAY_ID );
        cout <<data1<< endl;
        printf("\n" );
